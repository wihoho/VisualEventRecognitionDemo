...
def randomYoutubeIndices(numberFromEachClass):
    ...
    betterIndices = util.loadObject("/Users/GongLi/PycharmProjects/VisualEventRecognitionDemo/YoutubeVideos/recognition/correctVideos.pkl")
...
def buildClassifiers(distances, binaryLabels, trainIndices):
    global baseKernels
    global globalTrainIndices
    global classifiers


    globalTrainIndices = trainIndices
    ...
    for i in range(len(distances)):
        ...
        distance = distance ** 2
        trainingDistances = util.sliceArray(distance, trainIndices)
    ...
    for classNum in range(binaryLabels.shape[1]):
        ...
        TrainingLabels = [thisClassLabels[index] for index in trainIndices]

        classClassifiers = []
        for m in range(len(baseKernels)):
            baseKernel = baseKernels[m]
            Ktrain = util.sliceArray(baseKernel, trainIndices)

            clf = SVC(kernel="precomputed")
            clf.fit(Ktrain, TrainingLabels)
            classClassifiers.append(clf)

        classifiers.append(classClassifiers)

    return classifiers, gramma0


# predict test videos
def predict(testIndices):
    global classifiers
    global baseKernels
    global globalTrainIndices

    scores = []
    for numOfClass in range(len(classifiers)):
        classClassifiers = classifiers[numOfClass]

        finalTestScores = np.zeros((len(testIndices))).reshape((1, len(testIndices)))

        for m in range(len(baseKernels)):
            baseKernel = baseKernels[m]
            Ktest = baseKernel[np.ix_(testIndices, globalTrainIndices)]

            clf = classClassifiers[m]
            dv = clf.decision_function(Ktest)
            finalTestScores = np.vstack((finalTestScores, dv.reshape((1, len(testIndices)))))
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        trainIndices, testIndices = randomYoutubeIndices(18)
